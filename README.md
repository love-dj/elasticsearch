...
```
composer require love-dj/elasticsearch dev-main
```
...
```php
    ...
    Es::init($config)->setIndex('products')->get();
```
> 3、可以使用子类继承\think\facade\Es，通过重写clientBuilder方法和设置index来简化调用，同时可以在子类中对复杂的操作进行封装
```php
    class Es extends \think\facade\Es
    {
