...
class Es
{
    ...
    protected function clientBuilder()
    {
        ...
        if (isset($this->config['connection_pool'])) {
            $client->setConnectionPool($this->config['connection_pool']);
        }

        if (isset($this->config['selector'])) {
            $client->setSelector($this->config['selector']);
        }

        if (isset($this->config['serializer'])) {
            $client->setSerializer($this->config['serializer']);
        }

        if (isset($this->config['connection_retry_times'])) {
            $client->setRetries($this->config['connection_retry_times']);
        }
    ...
    public function first()
    {
        ...
        if (isset($result['hits']['hits'][0])) {
            ...
            $data = array_merge($hit['_source'], isset($hit['highlight']) ? ['highlight' => $hit['highlight']] : []);
        }
        return $data;
    }
    ...
    public function paginator(int $page = 1, int $size = 10)
    {
        ...
        $list = array_map(function ($hit) {
            return array_merge($hit['_source'], isset($hit['highlight']) ? ['highlight' => $hit['highlight']] : []);
        }, $result['hits']['hits']);
